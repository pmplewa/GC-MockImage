...
parser.add_argument("--data", "-d", type=str_isfile, default="stars.ndjson", metavar="string",
    help="data file (default: stars.ndjson)")

parser.add_argument("--image_size", "-n", type=int, default=100, metavar="int",
    help="image size in pixels (default: 100)")
parser.add_argument("--pix_scale", "-s", type=float, default=0.013, metavar="float",
    help="pixel scale in arcsec per pixel (default: 0.013)")

parser.add_argument("--distance", "-R0", dest="R0", type=float, default=8.34, metavar="float",
    help="distance to the SMBH in kpc (default: 8.34)")
parser.add_argument("--mass", "-M0", dest="M0", type=float, default=4.40, metavar="float",
    help="mass of the SMBH in 10^6 M_sun (default: 4.40)")

parser.add_argument("--wavelength", "-w", type=float, default=2e-6, metavar="float",
    help="wavelength in meters (default: 2e-6)")
parser.add_argument("--aperture", "-D", type=float, default=8, metavar="float",
    help="aperture in meters (default: 8)")

parser.add_argument("--halo_frac", type=float_isfraction, default=0, metavar="float",
    help="strength of seeing halo as fraction (default: 0)")
parser.add_argument("--halo_fwhm", type=float, default=0.5, metavar="float",
    help="size of seeing halo in arcsec (default: 0.5)")

parser.add_argument("--zero_point", "-zp", type=float, default=14, metavar="float",
...
def gen_image(t):
    ...
    for i, star in enumerate(stars):
        ...
        table["vx"][i] = -p.vx
        table["vy"][i] = p.vy
        table["vz"][i] = (p.vz*v_unit).to_value(units.km / units.s)
...
def gen_plot(hdu_list, m0=20, m1=14, labels=True, show=True, save=None):
    ...
    ax.imshow(image, origin="lower", interpolation="none", cmap="YlOrRd",
...
if __name__ == "__main__":
    parser.add_argument("--epoch", "-t", type=float, required=True, metavar="float",
        help="time of observation (year)")

    parser.add_argument("--output_fits", "-o", type=str, required=False, metavar="string",
    ...
    parser.add_argument("--nolabels", "-nl", dest="labels", action="store_false", default=True,
        help="do not label stars on the preview image")

    parser.add_argument("--output_preview", "-op", type=str, required=False, metavar="string",
